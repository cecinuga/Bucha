...
export default function OperaLayerSection(props: OperaLayerProps){
    ...
    return(
        <ParallaxLayer style={{backgroundColor:"white"}} factor={props.factor} offset={props.offset} speed={props.speed}>
            <Stack align="center">
                <Title className="font-primary" fw={200} pt={lg?"0.5rem":"2rem"} pb={lg?"0.5rem":"2rem"}>{props.title}</Title>
                {props.descs?.map((item, i) => 
                    <Group position="center" pb={lg?0:20}>
                        {props?.icons?.[i]}
                        <Card radius={"md"} bg={props?.bgs?.[i]} withBorder p={lg?40:50} w={"50%"} c="white">
                            <Card.Section>
                                <strong>{item}</strong>
                            </Card.Section>
                        </Card>  
